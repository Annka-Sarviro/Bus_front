...
const BusSeats = (props: BusConstructorProps) => {
    ...
    const checkStatus = (id: number | null | undefined): string | undefined => {
        const seat = props.seats?.find(seat => seat.seat_number === id);
    ...
    function BusSeat({
        ...
        return (
            <button
                id={id}
                onClick={() => (seatNumber ? handleClick(seatNumber, props?.floor) : null)}
    ...
    return (
        ...
        >
            {busSeats.map((row, index) => (
                <BusRow key={index} seats={row} small={props.small} vertical={props.vertical} />
            ))}
        </Box>
    );
};
