...
export const CurrencySelect = () => {
    ...
    const [selectedIndex, setSelectedIndex] = React.useState<string>('EUR');
    ...
    const getCurrency = () => {
        const data = currency.filter(item => item.title === selectedIndex);
            ...
            >
                {currency?.map(item => (
                    <MenuItem
                        key={item.id}
                        disabled={item.id === 0}
                        selected={item.title === selectedIndex}
                        onClick={event => handleMenuItemClick(event, item.title, item.id)}
