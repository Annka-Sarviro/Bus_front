...
import BusConstructor from '@/components/common/BusConstructor/BusConstructor';
...
export const SeatsBooking = ({
    ...
    useEffect(() => {
        setFirstFloorSeats(data?.seats?.first_flour_seats);
    }, [data?.bus?.first_floor_seats]);
    useEffect(() => {
        setSecondFloorSeats(data?.seats?.second_flour_seats);
    }, [data?.bus?.second_floor_seats]);
    ...
    const handleCheck = (seatNumber: number, floor: number) => {
        ...
        if (floor === 1) {
            setFirstFloorSeats((prevState: ISeat[] | undefined) => {
                const updatedSeats = prevState?.map(seat => {
                    if (seat.seat_number === seatNumber) {
                        if (seat.status === 'selected') {
                            return { ...seat, status: 'empty' as const };
                        }
        ...
        if (floor === 2) {
            setSecondFloorSeats((prevState: ISeat[] | undefined) => {
                const updatedSeats = prevState?.map(seat => {
                    if (seat.seat_number === seatNumber) {
                        if (seat.status === 'selected') {
                            return { ...seat, status: 'empty' as const };
                        ...
                        <Box>
                            ...
                            >
                                <Tab
                                    ...
                                    {...a11yProps(0)}
                                />
                                {data?.bus?.second_floor_seats_count > 0 && (
                                    <Tab
                            ...
                            <CustomTabPanel value={value} index={0}>
                                <Box>
                                    <FormGroup sx={{ display: 'flex', flexDirection: 'row' }}>
                                        <BusSeats
                                            rows_1={data?.bus?.rows_1}
                                            enter_1={data?.bus?.enter_1}
                                            rows_2={data?.bus?.rows_2}
                                            rows_3={data?.bus?.rows_3}
                                            enter_2={data?.bus?.enter_2}
                                            seats={firstFloorSeats}
                                            seats_start={1}
                                            handleCheck={handleCheck}
                                            floor={1}
                                            is_wc={data?.bus?.wc ? 'yes' : 'no'}
                                            is_wc_2={data?.bus?.wc_2 ? 'yes' : 'no'}
                                            wc_row_1={data?.bus?.wc_row_1}
                                            wc_row_2={data?.bus?.wc_row_2}
                                        />
                                    </FormGroup>
                                </Box>
                            </CustomTabPanel>
                            <CustomTabPanel value={value} index={1}>
                                <Box>
                                    <FormGroup sx={{ display: 'flex', flexDirection: 'row' }}>
                                        <BusSeats
                                            rows_1={data?.bus?.rows_4}
                                            enter_1={data?.bus?.enter_3}
                                            rows_2={data?.bus?.rows_5}
                                            seats={secondFloorSeats}
                                            seats_start={data?.bus?.first_floor_seats_count + 1}
                                            handleCheck={handleCheck}
                                            floor={2}
                                        />
                                    </FormGroup>
