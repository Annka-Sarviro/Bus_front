...
import { usePopular } from '@/lib/store';
...
export const SearchRoutForm = ({
    ...
    const selectRoutsTo = usePopular(state => state.selectRoutsTo);
    const selectRoutsFrom = usePopular(state => state.selectRoutsFrom);
    ...
    const Search = async (event: React.SyntheticEvent) => {
        ...
        try {
            const response = await axios.get(
                `${process.env.NEXT_PUBLIC_BACK_URL}/journey?from_city=${values.from}&to_city=${values.to}&from_date=${values.date}`
            );

            if (response.status === 200) {
                setSearchJourney(response.data.data.result);
            } else return [];
        } catch (error) {
            console.log(error);
        }
