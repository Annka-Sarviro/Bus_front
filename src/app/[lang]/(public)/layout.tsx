...
import { getAdminStatus, getUserInfo } from '@/lib/auth';
import { IContact, IPhone } from '@/interface/IContact';

const getPopularRouts = async (lang: Locale) => {
    try {
        const response = await axios.get(
            `${process.env.NEXT_PUBLIC_BASE_URL}/routes/popular_routes`
        );
...
const getContact = async (lang: Locale): Promise<IPhone[]> => {
    try {
        const response = await axios.get<IPhone[]>(
